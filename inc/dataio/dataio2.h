...
//
// 04.11.13  iss51-1
//  Переименован в dataio2.h; модификация для независимого формирования и передачи 
//  исходящих пакетов: исключен Layer1, в Layer3 передается указатель на внешний
//  буфер для формирования исходящего пакета.
//  Из Layer2 исключена ссылка на объект Layer1.
//  Адаптировано под ведущее устройство, формирующее запросы к ведомым.
//
	...
	class	DataIOLayer2
	{
      ...
      //-----------------------------------------------------------------------
          ...
          resetRxBuffers();	clearErrors();
			...
			void finishTxPacket(LR2_PACKET* p2)
			{
...
//			LR2_PACKET	txPacket;			// буфер для передаваемого пакета
		...
		public:
			...
			void startTxPacket(unsigned destAddress, LR2_PACKET* buffer)
			{                        
				txIndex = 0;                                  
				txPacket = buffer;
			...
			uint16_t finishTxPacket(void)
			{
				if(txIndex)
				{
					...
					lr2Ptr->finishTxPacket(txPacket);
					txIndex = 0;
