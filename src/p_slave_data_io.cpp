...
//
// 1. Управление автоматом опроса ведомых устройств:
//    - пуск/останов опроса
//    - периодическое выполнение автомата
// 2. При приеме сообщений от ведомого устройства вызывает соответствующие
//    обработчики автомата. 
// 3. При приеме данных от ведомого устройства передает блок
//    в контейнер блоков данных для передачи на ВУ.
...
// Обработка принятого блока данных - записывает блок в контейнер БД
//
// Стек вызовов: 
//   handleMessageData() -> SlavesPoll::handleData() -> dataHandler()
// SlavesPoll::handleData() при приеме БД выполняет обработку состояния автомата 
// и вызывает dataHandler(): 
//   a) при соответствии адреса отправителя текущему запросу
//   b) если БД не повторный
//
